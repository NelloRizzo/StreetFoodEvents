...
import { EventUserModel, EventUserTransactionModel } from './models';
import { registerSync } from './sync.service';

type CreateEventUserTransactionInput = {
    eventUserId: string | Types.ObjectId;
    type: 'top-up' | 'purchase' | 'refund' | 'adjustment' | 'transfer-in' | 'transfer-out';
...
export async function createEventUserTransaction(input: CreateEventUserTransactionInput) {
    ...
    try {
        ...
        if (!eventUser) throw new EventUserTransactionError('Event user not found');
        if (!eventUser.isActive) throw new EventUserTransactionError('Event user is not active');

        const balanceChange = input.direction === 'credit' ? input.amount : -input.amount;
        const nextBalance = eventUser.balance + balanceChange;
        if (nextBalance < 0) throw new EventUserTransactionError('Insufficient event currency balance');
        ...
        await registerSync('EventUserTransaction', transaction._id);

        if (ownSession) {
            await session.commitTransaction();
        }

        return { eventUser, transaction };
    } catch (error) {
        if (ownSession) {
