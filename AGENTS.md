...
- **Pagina dedicata al tracking per stand** (`StandTrackingPage`, porta `events/:eventId/stands/:standId/tracking`): route **standalone senza layout** (niente navbar, come `ordersqueue`); testata con **banner** (`stand.coverImage`), **logo** (`logo ?? coverImage`), **nome** e **numero** dello stand (da `stand.numbers` match per `eventId`) **nel tema dell'evento** (`useEventTheme` + CSS vars `--color-brand/brand-deep/highlight`, fallback navy); il corpo contiene **solo il tracking abilitato automaticamente** — nuova variante **`page`** di `OrderTrackingModal` (card embedded senza overlay/backdrop, **nessun** ascolto di `onTrackingClear` né Escape: la pagina resta montata finché c'è un ordine; basta `selectedStandId` + `open`). In assenza di ordini mostra il placeholder **"Preparazione ordine in corso..."** (`.empty`/`.emptyText`/`.emptyHint`, riutilizzati cancellando l'empty state `inline`). Nuovo file `StandTrackingPage.module.scss`.
...
- **GOTCHAS**: `OrderTrackingModal` ora ha solo varianti `'standalone' | 'page'` (l'ex `'inline'` è sparito — nessun call site rimasto); per la variante standalone il `standId` si omette (dropdown) mentre per `page` è sempre esplicito; il placeholder della variante `page` appare quando non c'è `kiosk.order`.
- Verifica: frontend build (tsc+vite) ✓, 43 test vitest ✓, lint 0 errori (13 warning pre-esistenti). Backend non toccato. Solo file cloud: **nessuna rigenerazione di `distro/local-app.tar` necessaria**. COMMIT + PUSH su `main`.
