...
- Quando un ordine viene effettuato, in cassa viene mostrata la ricevuta. **Chiosco ordini**: postazione aggiuntiva (attivabile per-browser, a scelta dell'admin stand/evento/platform) che mostra il QR dell'ultimo ordine con il riepilogo; l'utente fotografa il QR → pagina pubblica `/track/:id` con riepilogo e notifica (suono/PRONTO) quando l'ordine è pronto.
