...
- **Implementato (Set 2026)**: il modulo cartaceo stampabile è `docs/ADESIONE_STAND.md`; è stato inoltre aggiunto il **modulo di adesione generato dall'evento** (`AdhesionForm`) esposto come pagina stampabile sia pubblica (`/events/:eventId/adhesion-form`) che di gestione (`/admin/events/:eventId/adhesion-form`), con generazione automatica dai dati dell'evento e avviso di obsolescenza quando l'evento cambia. Da settembre 2026 è disponibile anche il **wizard di compilazione elettronico** (`StandAdhesion`, API `/api/events/:eventId/adhesions`, workflow `draft → submitted → approved/rejected`): pagina pubblica anonima `StandAdhesionWizardPage` (`/events/:eventId/stand-adhesion`, gate regolamento, accesso anonimo con access token in `localStorage` per stand NUOVI; percorso admin `/admin/events/:eventId/stand-adhesion` per admin/owner) e gestione/approvazione `AdhesionsManagePage` (`/admin/events/:eventId/adhesions`, **solo event-admin** — platform-admin escluso). Al submit dei nuovi stand il sistema crea (o riusa) un utente inattivo con invito email; all'approvazione crea lo Stand con numero progressivo e assegna `stand-admin`. Vedi CHANGELOG Settembre 2026 e ARCHITECTURE "Modulo di adesione stand" / "Wizard di adesione stand".
- **Aperti (wizard elettronico)**: **pagamenti online** del prezzo di partecipazione/caparra — oggi sono campi informativi su `Event` (`participationFee`/`deposit` con scadenze `participationFeeDeadline`/`depositDeadline`) accettati con checkbox nel wizard; resta da integrare un Payment Gateway. Dipende anche dal futuro ruolo `stand-owner` dedicato (oggi si riusa `stand-admin`).
