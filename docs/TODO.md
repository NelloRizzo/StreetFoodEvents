...
- **Implementato (Set 2026)**: il modulo cartaceo stampabile è `docs/ADESIONE_STAND.md`; è stato inoltre aggiunto il **modulo di adesione generato dall'evento** (`AdhesionForm`) esposto come pagina stampabile sia pubblica (`/events/:eventId/adhesion-form`) che di gestione (`/admin/events/:eventId/adhesion-form`), con generazione automatica dai dati dell'evento e avviso di obsolescenza quando l'evento cambia. Vedi CHANGELOG Settembre 2026 e ARCHITECTURE "Modulo di adesione stand".
- **Da implementare (wizard elettronico)**: raccolta dati dell'aderente direttamente in app (modello `StandAdhesion`, API, stati) per sostituire la compilazione cartacea. Resta dipendente da: ruolo `stand-owner` e Payment Gateway (per pagamenti online) non ancora implementati.
