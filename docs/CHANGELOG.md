...
## Settembre 2026
- **Chiosco QR ordini + pagina track ordine con notifica "pronto"**: ① **backend** — due nuovi endpoint pubblici (registrati PRIMA di `authMiddleware` in `orders.routes.ts`, accanto a `/orders/:orderId/receipt` e `/orders/stand/:standId/ordersqueue`): `GET /api/orders/:orderId/track` (`getOrderTrack`: minimi dati `{ id, orderNumber, status, isGift, readyAt, createdAt, eventId, standId, eventName, standName, items[{ productName, quantity, stationName }] }` — niente dati cliente né prezzi, valore anche per ordini già completati/cancellati) e `GET /api/orders/stand/:standId/kiosk-recent` (`getStandKioskRecent`: filtra `confirmed/preparing/ready`, opzione `eventId`, restituisce l'ultimo ordine per `orderNumber: -1`, `queueCount`, `trackUrl` costruita da `req.query.url` → fallback origin/request e **QR data URL server-side** con la lib `qrcode` — il frontend non ha lib QR). ② **frontend** — pagina pubblica **`TrackOrderPage`** (`/track/:orderId`, fullscreen, polling 5s): schermata "PRONTO!" verde quando l'ordine passa a `ready` con **beep Web Audio** (2 doppi toni) + **Notification API opzionale** (pulsante "Abilita notifiche", permesso richiesto solo su click), stati completato/annullato/in lavorazione, riepilogo "Cosa hai ordinato"; pagina **`OrderKioskPage`** (`/events/:eventId/stands/:standId/kiosk`, fullscreen): fino all'attivazione mostra il gate che richiede autorizzazione (platform/scoped event-ol 1=eventId, oppure stand incluso in `/auth/me/stands`); l'admin clicca **"Attiva chiosco su questo browser"** → `localStorage['sfe_kiosk_<standId>']`, poi la pagina fa polling (`fetchStandKioskRecent` con `url = window.location.origin`) e mostra **QR grande + numero ordine + riepilogo + badge "N ordini in coda"**, con pulsante Disattiva. Helper + tipi in `lib/orders.ts` (`fetchOrderTrack`, `fetchStandKioskRecent`, `OrderTrack`, `KioskState`, `KIOSK_ACTIVE_KEY`). Route aggiunte a `PublicLayout` con `hideChrome` esteso a `/track` e `/kiosk`. Test: +4 in `orders.test.ts` (track pubblico minimale senza dati cliente/prezzi; 400/404 track; kiosk-recent ultimo in lavorazione + QR + filter evento; kiosk vuoto + 404/400), suite backend 46 file / **425 test ✓**, typecheck ✓, lint 0 errori; frontend build (tsc+vite) ✓, 43 test vitest ✓, lint 0 errori (13 warning pre-esistenti). TODO.md punto 2 → fatto. Solo file cloud: **nessuna rigenerazione di `distro/local-app.tar` necessaria**.
