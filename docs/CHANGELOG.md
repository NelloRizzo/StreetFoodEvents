...
- Esporta per social dal menu stand (cloud, solo frontend): pulsante "Esporta social" nella header della pagina menu pubblico dello stand (`/events/:eventId/stands/:standId`), visibile solo a utenti autorizzati — platform admin, admin evento (`adminEventIds`) o admin dello stand (ruolo stand-scope via `GET /auth/me/stands`). Apre un modale (`SocialMenuExport`) che genera e scarica un **poster PNG 1080x1350** (4:5, formato social) con: logo della manifestazione + nome + data, **banner (`coverImage`) dello stand** come fascia sotto la testata (se presente) con il **logo dello stand** sovrapposto (fallback alla cover, o iniziale del nome), nome + slogan, **i prodotti in vendita con la foto thumbnail di copertina** (placeholder con iniziale quando manca, SENZA prezzo, max 10 nel poster), luogo/date dell'evento e **QR code verso il menu dello stand** (dalla scan URL condivisa `GET /api/stands/:standId/qrcode`); i colori seguono il tema dell'evento. Insieme al poster il modale offre il **testo del post** (caption pronta con nome stand, evento, data, luogo e lista prodotti senza prezzi) con copia negli appunti e download `.txt`, più il QR del menu scaricabile separatamente. Generazione poster 100% client-side (canvas, immagini `crossOrigin`), nessun cambiamento backend. Helpers puri in `frontend/src/lib/socialMenu.ts` (`formatEventDateRange`, `buildSocialMenuCaption`) con 9 test frontend; typecheck, build e vitest verdi. Nessun tocco a `.local/`: nessuna rigenerazione di `distro/local-app.tar` necessaria.
