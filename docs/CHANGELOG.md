...
- Nuove tipologie POI: l'`iconType` di `POI` ammette ora anche `cinema`, `relax` e `divertimento`. Aggiunte al selettore Icona del form POI in `EventDetailPage` (🎬 Cinema, 🧘 Relax, 🎢 Divertimento) e alla resa dei marker in `EventMapPage`.
