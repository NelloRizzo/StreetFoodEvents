...
- **Fix "Stand collegato" per nome nel wizard adesione**: il campo "Stand collegato" dei gestori evento accettava solo l'ObjectId, ma l'operatore digita il **nome** dello stand → `Cast to ObjectId failed` alla creazione/modifica dell'adesione. Il backend ora risolve il riferimento (`resolveStandReference`) cercando lo stand per **nome** (case-insensitive) tra gli stand dell'evento, oltre che per id; nome inesistente → 400 "Nessuno stand trovato…". Il frontend popola il `<datalist>` (era referenziato ma mai renderizzato) con gli stand dell'evento per scegliere per nome. Test: +2 in `integration-stand-adhesions.test.ts`, suite backend **374 test OK**, typecheck + lint OK, frontend build + lint OK. Nessun tocco a `.local/`: nessuna rigenerazione di `distro/local-app.tar` necessaria.
