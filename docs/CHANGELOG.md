...
- **Modulo di adesione — logo ed evidenziazione moneta**: nella generata "Sezione D — Moneta dell'evento" il nome della moneta custom è ora **evidenziato** e affiancato dal **logo della moneta** (`currencySymbol.url`, `<img>` width 72) quando presente, altrimenti dall'**iniziale in circoletto** (`Ⓣ`). Il sanitizer backend ora consente il tag `<img>` (src/alt/width/height, scheme http/https) così il logo sopravvive al salvataggio PATCH dalla pagina di gestione; `RichEditor` supporta le immagini (estensione `@tiptap/extension-image`, pulsante "Immagine (URL)" in toolbar — utile a tutte le descrizioni HTML). Test: nuovo test nel file adhesion-form (logo embedded + preservato via PATCH); suite backend 314 test OK, frontend build + 41 test vitest OK. Solo cloud: nessuna rigenerazione di `distro/local-app.tar` necessaria.
