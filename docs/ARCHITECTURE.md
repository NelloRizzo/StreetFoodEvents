...
- **Modello**: `StandAdhesion` (collezione `standadhesions`), unique index `{ eventId, standId }` — un'adesione per evento per stand. Denormalizza i dati dello stand (`standName`, `standType`, `coverImage`, `logo`, `pitch`, `contact`) così lo storico resta valido anche se lo stand cambia. `products` (array di subdocument senza `_id`), `haccp.confirmed`, `energyNeeds` (righe `{ description, powerKw, connectionType }`, optional), flags di accettazione (`acceptsPointLight`, `participationFeeAccepted`, `depositAccepted`, `regulationAccepted`, `exclusionAccepted`, `energyConditionsAccepted`), `signature` (str) e `status` + `submittedAt`. `review` subdocument per il rifiuto (`{ status, note, reviewedBy, reviewedAt }`).
- **`Event.participationFee`/`deposit` (Number, default null)**: sono SOLO informativi, esposti in `toEventResponse` e copiati da `duplicateEvent`. Il wizard li mostra come importi + checkbox di accettazione: NESSUN pagamento online in questo step (Payment Gateway fuori scope).
- **Transizioni**: `draft → submitted → approved/rejected`. `submit` valida la completezza (`completenessErrors` → 400, campi: standName, haccpConfirmed, acceptsPointLight, participationFeeAccepted, depositAccepted, regulationAccepted, exclusionAccepted, signature non vuota); il edit di una `submitted` la riporta a `draft` con `submittedAt: null`; `approved` → 409 su edit e withdraw; `withdraw` solo da `submitted`.
- **Guard**: approvare/rifiutare = `hasRole(['event-admin','platform-admin'], { eventParam: 'eventId' })`. TUTTE le altre route (CRUD/mine/submit/withdraw) usano authMiddleware + check custom: `isAdminForEvent` (replica la logica di `hasRole` per ruoli platform/event, incluso `eventId: null`/assente) OPPURE possesso dello stand (ruoli stand-scope con `roleId`/`standId` attivi). Chi non è admin né possessore → **404** (nessun leak informativo). `GET /adhesions/mine` → `{ item: null }` quando non esiste, `{ item }` altrimenti (anche rejected).
- **Approvazione NON auto-collega**: a differenza di `duplicateEvent`/numerazione, `approve` non tocca `Stand.eventIds` — il collegamento dello stand all'evento resta un'azione manuale separata.
- **Frontend lint GOTCHA** (lezione da `AdhesionsManagePage`): la regola `react-hooks/set-state-in-effect` flagga `void load()` / `void fetchItems()` invocati sincronicamente nel corpo di un `useEffect`, anche se la funzione è async. Pattern accettato nel repo: catene `.then(...)` INLINE nel useEffect (come `AdhesionFormManagePage`, `DashboardPage`); per il re-fetch post-azione (approve/reject) si tiene la funzione `useCallback` usata fuori dal primo effect. `ConfirmModal` NON accetta `children` → la nota di rifiuto passa da `variant="prompt"`.
